...
struct InputMLEMD 
{
    std::vector<float> features; // 2-d features:{ dphi_EMD, eta_track}
};
...
public:
    ...
    PtResult ComputeMLEMD(const InputMLEMD& in) const; // machine learning model trained for the EMD method, the features are 2-d features: { dphi_EMD, eta_track}, and the internal model input is { 1/abs(dphi_EMD), eta_track}
    ...
    // ===== setters for global-ish configuration =====
    // 0 = Projected(fixed radius 93.5cm), 1 = InnerFace center, 2 = geometric center
    void setClusterRecoMode(int mode)
    {
        if (mode < 0 || mode > 2) {
            std::cerr << "ERROR: ClusterRecoMode must be 0/1/2" << std::endl;
            return;
        }
        ClusterRecoMode = mode;
    }
    int  getClusterRecoMode() const { return ClusterRecoMode; }
    ...
    void setMLEMDInfer(int scenario, InferFn fn);
    ...
    void setMLEMDStandardizer(int scenario, std::vector<float> mean, std::vector<float> scale);
    ...
    void setMLEMDBaseParam(int scenario, ResidualBaseParam param);
...
private:
    ...
    static float ComputeResidualBasePt(float proxy,
                                   float eta,
                                   const ResidualBaseParam& param);
    static ResidualBaseParam GetDefaultMLEMDBaseParam(int scenario);

private:
    PtCalculatorConfig m_cfg;

    int getScenario(double bending_angle = 0.1) const
    {
        int TrkrCharge = -1; // default negative-1 charge, can be set to positive(=1) if needed.
        if (bending_angle > 0)
            TrkrCharge = +1;
        else if (bending_angle < 0)
            TrkrCharge = -1;

        // charge: -1 +1, ClusterRecoMode: 0/1/2 -> 6 scenarios
        int track_scenario = TrkrCharge*10 + TrkrCharge*ClusterRecoMode;
        if (!((track_scenario >= -12 && track_scenario <= -10) ||
              (track_scenario >= 10  && track_scenario <= 12)))
        {
            std::cerr << "[PtCalculator] Invalid scenario = "
                      << track_scenario
                      << " (charge=" << TrkrCharge
                      << ", mode=" << ClusterRecoMode << ")"
                      << std::endl;
        
            return -999;   // 明确的 invalid flag
        }

        return track_scenario;
    }

    // 0 = Projected(fixed radius 93.5cm), 1 = InnerFace center, 2 = geomtric center
    int ClusterRecoMode = 0; 

    // ML Model infer functions
    std::unordered_map<int, InferFn> m_mlEMD_infer;
    ...
    std::unordered_map<int, std::vector<float>> m_mlEMD_mean, m_mlEMD_scale;
    std::vector<float> m_mlEproj_mean, m_mlEproj_scale; 
    std::vector<float> m_mlCombined_mean, m_mlCombined_scale;

    std::unordered_map<int, ResidualBaseParam> m_mlEMD_base_param;
